...
public class GameInstance {
	...
	public static void onStart() {
		new GameStart(Main.getPlugin(), Main.getPlugin().getConfig().getInt("General.WaitingTimer"))
	...
	public static void end() {
		if (TeamsB.state.size() == 1) {
			String wt = TeamsB.state.get(0);
			Bukkit.broadcastMessage(ChatColor.translateAlternateColorCodes('&', Main.getPlugin().getConfig().getString("Messages.Winner").replace("{team}", wt)));
			GameBool.Finish = true;
			for(Player p : Bukkit.getOnlinePlayers()){
				p.getInventory().setItem(Main.getPlugin().getConfig().getInt("Items.Leave.Slot"), 
						CustomItem.CustomItem2(Main.getPlugin().getConfig().getString("Items.Leave.Material"),
								Main.getPlugin().getConfig().getInt("Items.Leave.Amount"), 
								(byte) Main.getPlugin().getConfig().getInt("Items.Leave.Byte"), 
								Main.getPlugin().getConfig().getString("Items.Leave.Displayname"), 
								Main.getPlugin().getConfig().getBoolean("Items.Leave.Glow"), Main.getPlugin().getConfig().getString("Items.Leave.Lore")));
			}
			if (wt.equals("blue")) {
				for (String blue_names : TeamList.blue) {
					Player blue = Bukkit.getPlayer(blue_names);
				    WinMation.WinAnimation(blue);
				}
			} else 
			if (wt.equals("green")) {
				for (String green_names : TeamList.green) {
					Player green = Bukkit.getPlayer(green_names);
				    WinMation.WinAnimation(green);
				}
			} else 
			if (wt.equals("red")) {
				for (String red_names : TeamList.red) {
					Player red = Bukkit.getPlayer(red_names);
				    WinMation.WinAnimation(red);
				}
			} else 
			if (wt.equals("yellow")) {
				for (String yellow_names : TeamList.yellow) {
					Player yellow = Bukkit.getPlayer(yellow_names);
				    WinMation.WinAnimation(yellow);
				}
			}
		}
	}
