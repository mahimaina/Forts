...
public class State implements Listener{
	...
	public void onPing(ServerListPingEvent e){
		if(GameBool.isStarted() && !GameBool.Finish){
			e.setMotd(ChatColor.translateAlternateColorCodes('&', Main.getPlugin().getConfig().getString("MOTD.Running")));	
		} else if(GameBool.Finish && GameBool.isStarted()){
			e.setMotd(ChatColor.translateAlternateColorCodes('&', Main.getPlugin().getConfig().getString("MOTD.Ending")));
		} else {
